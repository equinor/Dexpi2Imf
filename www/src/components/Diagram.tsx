...
export default function Diagram() {
  ...
  function checkOnlyInsideBoundary() {
    ...
    const hasPipingBoundary = Array.from(piping.current).some((pipe) =>
      pipe.classList.contains("boundary"),
    );
    const hasNodeBoundary = Array.from(nodes.current).some((node) =>
      node.classList.contains("boundary"),
    );
    const hasInsideBoundary = Array.from(nodes.current).some((node) =>
      node.classList.contains("insideBoundary"),
    );
